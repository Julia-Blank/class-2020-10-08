...
draws #%>%
 # filter(state %in% c("Pennsylvania", "Ohio")) %>%
...
elections %>%
  ...
  mutate(both = map_lgl(simulations, ~ .$Ohio & .$Pennsylvania > 50)) %>%
  summarize(oh_avg = mean(oh_win), pa_avg = mean(pa_win), both_avg = mean(both))
